...
def known_confusion_matrix_calc(cluster_df, annotations_csv, output_heatmap):
    ...
    if len(merged_df) == 0:
        print("WARNING: No annotations map back to embeddings. Heatmap of known sounds cannot be produced.")
    else:
        ...
        cluster_merged = cluster_merged.groupby('cluster_number').apply(lambda x: x.sort_values(['percentage'], 
